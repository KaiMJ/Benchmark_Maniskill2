...
def convert(input_path, output_path):
    ...
    img.save_render(filepath=f'{output_path}/texture_kd.png')
    if bpy.app.version[0] >= 4:
        bpy.ops.wm.obj_export(filepath=f"{output_path}/mesh.obj")
    else:
        bpy.ops.export_scene.obj(filepath=f"{output_path}/mesh.obj")

    with open(f'{output_path}/mesh.mtl', 'a') as f:
